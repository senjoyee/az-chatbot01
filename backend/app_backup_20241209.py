...
scoring_profile = ScoringProfile(  
    ...
    text_weights=TextWeights(weights={  
        ...
        "customer_name": 4,  # Medium weight for customer name
...
async def upload_files(
    ...
    customer_name: Optional[str] = Form(None)  # Make it optional
):
    try:
        # Add size validation
        ...
        for file in files:
            ...
            try:
                # Store customer name in blob metadata
                metadata = {}
                if customer_name:
...
async def process_file_async(event: BlobEvent):
    ...
    try:
        if event.event_type == "Microsoft.Storage.BlobDeleted":
            ...
            try:
                # Download the file and get blob metadata
                logger.info(f"Downloading file: {file_name}")
                temp_file_path = await storage_manager.download_file(file_name)
                blob_client = blob_service_client.get_container_client(BLOB_CONTAINER).get_blob_client(file_name)
                blob_properties = blob_client.get_blob_properties()
                customer_name = blob_properties.metadata.get("customer_name") if blob_properties.metadata else None
    ...
    try:
        for i, document in enumerate(documents_in, 1):
            ...
            document_obj = {
                ...
                "customer_name": document.metadata.get("customer_name", "unknown"),
                "last_update": datetime.utcnow().isoformat() + "Z"
            }
