...
$buildJobs = @(
    ...
    Start-Job -Name "BuildProxy" -ScriptBlock {
        param($workDir)
        Set-Location $workDir
        $output = docker build -t jssapmscr.azurecr.io/proxy:latest ./proxy 2>&1
        $output | Out-String
    } -ArgumentList $currentDir
)
...
$pushJobs = @(
    ...
    Start-Job -Name "PushProxy" -ScriptBlock {
        $output = docker push jssapmscr.azurecr.io/proxy:latest 2>&1
        $output | Out-String
    }
)
...
$restartJobs = @(
    ...
    Start-Job -Name "RestartProxy" -ScriptBlock {
        $output = az webapp restart --name jscb-proxy-nginx --resource-group js-sapservices-tools 2>&1
        $output | Out-String
    }
)
